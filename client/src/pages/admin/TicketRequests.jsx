...
import {
  ...
  XCircleIcon, EyeIcon, UserCircleIcon, ClockIcon,
...
export default function TicketRequests() {
  ...
  const [forceApproveId, setForceApproveId] = useState(null);

  const { data, isLoading } = useGetAdminTicketRequestsQuery({ page, status: statusFilter || undefined });
  const { data: statsData, isLoading: statsLoading } = useGetAdminTicketRequestStatsQuery();
  ...
  const handleApprove = async (id, force = false) => {
    try {
      await approveTicket({ id, force }).unwrap();
  ...
  const handleReject = async (id) => {
    try {
      await rejectTicket({ id, reason: rejectReason || 'Rejected by admin' }).unwrap();
  ...
  return (
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-6 pb-10">
      <div>
        <h1 className="text-2xl font-bold text-white font-display">Ticket Requests</h1>
        <p className="text-zinc-400 text-sm mt-1">Review and manage all ticket registrations across the platform</p>
      </div>

      {/* Stats */}
      <motion.div variants={item} className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: 'Total Requests', value: stats.total, icon: TicketIcon, color: 'text-primary-400' },
          { label: 'Pending', value: stats.pending, icon: ClockIcon, color: 'text-amber-400' },
          { label: 'Approved', value: stats.approved, icon: CheckCircleIcon, color: 'text-emerald-400' },
          { label: 'Rejected', value: stats.rejected, icon: XCircleIcon, color: 'text-red-400' },
        ].map((s) => (
          <div key={s.label} className="glass rounded-2xl p-4">
            ...
            {statsLoading ? (
              ...
              <p className={`text-xl font-bold text-white font-display tabular-nums`}>{s.value?.toLocaleString?.('en-IN') || s.value || 0}</p>
            )}
          </div>
        ))}
      </motion.div>
      ...
      <motion.div variants={item} className="flex items-center gap-3 flex-wrap">
        <div className="relative flex-1 min-w-[200px] max-w-xs">
          ...
          <input
            type="text" placeholder="Search by name, event, ID..."
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/[0.04]">
                <th className="text-left px-4 py-3 text-xs text-zinc-500 font-medium uppercase tracking-wider">Registration ID</th>
                ...
                <th className="text-left px-4 py-3 text-xs text-zinc-500 font-medium uppercase tracking-wider">Ticket Status</th>
              ...
              {registrations.map((reg) => (
                <motion.tr key={reg._id} variants={item} className="border-b border-white/[0.02] hover:bg-white/[0.02] transition-colors">
                  <td className="px-4 py-3">
                    <span className="text-violet-300 font-mono text-xs">{reg.registrationId || reg._id?.toString().slice(-8)}</span>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                  ...
                  <td className="px-4 py-3">
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium ${
                      ...
                      reg.paymentStatus === 'pending' ? 'bg-amber-500/20 text-amber-300' :
                  ...
                  <td className="px-4 py-3">
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium ${
                      ...
                      {reg.ticketStatus?.replace('_', ' ') || 'pending_approval'}
                    </span>
                  </td>
                ...
                <div className="space-y-4 text-sm">
                  <div className="grid grid-cols-2 gap-3">
                    <div className="glass-sm p-3 rounded-xl">
                      <p className="text-[10px] text-zinc-500 uppercase">Registration ID</p>
                      <p className="text-xs font-mono text-violet-300 mt-0.5">{reg.registrationId}</p>
                    </div>
                    <div className="glass-sm p-3 rounded-xl">
                      <p className="text-[10px] text-zinc-500 uppercase">Transaction ID</p>
                      <p className="text-xs font-mono text-zinc-300 mt-0.5">{reg.transactionId || '—'}</p>
                    </div>
                    <div className="glass-sm p-3 rounded-xl">
                    ...
                    <div className="glass-sm p-3 rounded-xl">
                      ...
                      <p className="text-xs text-zinc-300 mt-0.5">{reg.organizer?.name || '—'}</p>
                    </div>
                    <div className="glass-sm p-3 rounded-xl">
                  ...
                  {reg.attendeeDetails?.college && (
                    <div className="glass-sm p-3 rounded-xl">
                      <p className="text-[10px] text-zinc-500 uppercase">College / Institution</p>
                      <p className="text-xs text-zinc-300 mt-0.5">{reg.attendeeDetails.college}{reg.attendeeDetails.department ? ` · ${reg.attendeeDetails.department}` : ''}{reg.attendeeDetails.year ? ` · ${reg.attendeeDetails.year}` : ''}</p>
                    </div>
                  )}
                  ...
                  <div className="flex items-center gap-2 pt-2">
                    ...
                    <span className={`px-2.5 py-1 rounded-full text-[10px] font-medium ${
                      reg.ticketStatus === 'approved' ? 'bg-emerald-500/10 text-emerald-400' :
                      reg.ticketStatus === 'pending_approval' ? 'bg-amber-500/10 text-amber-400' :
                      'bg-red-500/10 text-red-400'
                    }`}>
                      Ticket: {reg.ticketStatus?.replace('_', ' ')}
                    </span>
                  </div>
      ...
      <AnimatePresence>
        {forceApproveId && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setForceApproveId(null)}>
            <div className="fixed inset-0 bg-black/70 backdrop-blur-sm" />
            <motion.div
              initial={{ opacity: 0, scale: 0.95 }}
              animate={{ opacity: 1, scale: 1 }}
            ...
            >
              <div className="text-center mb-4">
                ...
                <h3 className="text-lg font-semibold text-white">Payment Not Completed</h3>
                <p className="text-sm text-zinc-400 mt-2">
                  This attendee has not completed payment yet. Force approval will issue a ticket without payment confirmation.
                </p>
              </div>
              <div className="flex items-center gap-3">
                <button
                  onClick={() => setForceApproveId(null)}
                  className="flex-1 px-4 py-2.5 rounded-xl bg-white/[0.04] border border-white/[0.06] text-zinc-300 text-sm font-medium hover:bg-white/[0.08] transition-all"
                >
                  Cancel
                </button>
                <button
                  onClick={() => handleApprove(forceApproveId, true)}
                  disabled={approving}
                ...
                >
                  {approving ? 'Approving...' : 'Force Approve'}
                </button>
              </div>
            </motion.div>
          </div>
